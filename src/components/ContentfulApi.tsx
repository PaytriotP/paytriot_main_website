...
export default class ContentfulApi {
   ...
   *
   */
  static async getPageContentBySlug(slug: any, options = defaultOptions) {
   ...
   *
   */
  static async getPaginatedSlugs(page: any) {
    ...
    const { total } = response?.data?.blogPostCollection || 0;
    const slugs = response?.data?.blogPostCollection?.items
      ? response?.data?.blogPostCollection?.items.map((item: any) => item.slug)
      : [];
   ...
   *
   */
  static async getPaginatedBlogPosts(page: any) {
    const queryLimit = 10;
    const skipMultiplier = page === 1 ? 0 : page - 1;
    const skip = skipMultiplier > 0 ? queryLimit * skipMultiplier : 0;
   ...
   *
   */
  static async getPostBySlug(slug: any, options = defaultOptions) {
    const variables = { slug, preview: options.preview };
   ...
   *
   * Set your desired page size in src/utils/Config:
   ...
   *
   * Set your desired recent post list size in src/utils/Config:
